...
| `CLIORA_GIT_ALLOWED_HOSTS` | ✅ **when `CLIORA_AGENT_RUNS_ENABLED` is true** | `["github.com"]` — **JSON, not comma-separated** | Which git hosts this deployment may reach. **Empty is the default and means no repository can be registered at all**, which is right for a deployment that has not thought about it yet — but the refusal reads "This deployment does not allow repositories on github.com", which sounds like a bug rather than an unset variable. ⚠️ **A bare `github.com` or `a.com,b.com` makes Central fail to start**, with a parse error that names the field and not the format. The node keeps its own list (`runner.git.allowed_hosts`) and **both must pass**: Central is the coarse filter, the node is the final authority |
...
| `CLIORA_PROVIDER_API_BASE` | — | unset | Overrides the provider API root. Empty means GitHub's own. **A deployment setting and never a repository column** — that is what keeps "where does Central connect" out of reach of data somebody can create, which is the SSRF shape this avoids by construction rather than by validation (ADR 0033 §3) |
| `CLIORA_PROVIDER_API_HOSTS` | — | `api.github.com` | Which hosts Central may call when opening a pull request. **Unlike `CLIORA_GIT_ALLOWED_HOSTS`, empty here does not mean "nothing"** — it falls back to the default, because this list ships with a value and clearing it is far more likely to be an accident than a decision to disable pull requests. The two defaults differ because the questions do: "which git hosts has this deployment approved" starts at none, while "where does GitHub's API live" has one right answer |
| `CLIORA_PROVIDER_MAX_CONCURRENT` | — | `2` | How many pull-request creations may be in flight |
| `CLIORA_PROVIDER_PENDING_LIMIT` | — | `50` | How many finished runs may wait for a pull request before the worker stops taking more and says so. **The bound matters more than the number**: a backlog nobody can see becomes fifty pull requests the moment a provider recovers |
