...
Visual direction: for **mobile only**, use a Porcelain-derived near-white canvas, white surfaces, deep text, and a restrained blue accent. Mobile terminal, code preview, warnings, and errors stay entirely light even when the OS prefers dark; #62 already approved that direction and it is not reopened here. Desktop retains its current theme policy. Current production Porcelain still gives xterm and Monaco dark surfaces, so only the versioned mobile theme-contract/ADR mechanism remains proposed for review. Use local system fonts with Traditional Chinese fallbacks, restrained visual variance and motion, practical list density, semantic warning/danger colors, visible focus, minimum 44×44 px controls, and no remote fonts or assets.
