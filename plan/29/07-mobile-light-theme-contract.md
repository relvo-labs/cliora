...
同色相的 normal 與 bright 必須互相可分辨（≥1.3:1），否則強調等於沒發生。
...
| 色相 | normal | 對比 | bright | 對比 | normal/bright 互比 |
|---|---|---:|---|---:|---:|
...
| 同色相 normal／bright 互相可分辨 | ≥1.3:1 | 全部主題 |

「靠近背景的那兩個」由每個主題**具名宣告**（例如 `TERMINAL_DIM_ANSI: Record<ThemeId, [string,string]>`），
**不由明暗推斷**——推斷會在下一個主題加進來時安靜地選錯兩個。

現行五個深色主題在這組規則下**已經全部通過**（§0.1 的表：14 色 5.79–16.02，兩個 dim 色 1.50–3.93），
所以這條測試加進去不會讓既有主題變紅。這一點已經算過，不是假設。
