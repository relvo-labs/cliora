...
> **狀態：實作完成。** 2026-08-12 開立。
> 進度、證據與十處與計畫不同的地方，全在
> [`09-implementation-status.md`](./09-implementation-status.md)——**與本檔不一致時以那份為準**。
> 尚未收尾的兩項：瀏覽器 stack 的 V1 像素比對與 DB-backed e2e，待有 PostgreSQL 的環境重跑。
>
> *（2026-08-16 `CV-00` 更正：這一行原本寫「計畫已定稿，尚未開工」，
> 而 `09` 從實作完成那天起就寫著「實作完成」。讀計畫的人先讀 README，
> 所以那句過期字串讓 `research/03` 的第一版把整個 `PX-2` 當成待辦的一期。）*
>
