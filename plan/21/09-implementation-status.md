...
19. 🔴 **`DV-10` 我做了徽章和完成面板，漏了計畫 §6 的卡片編輯器——而那讓 `pull_request` 從 console 走不通。**
    使用者選了 `pull_request`、按派工，拿到「以合併請求交付必須指定目標分支」，
    然後**在整個 console 裡找不到任何一個地方可以設定它**。
    `target_branch` 連一個輸入框都沒有；`base_branch` 是唯讀顯示；
    Tag 與機密也只能看不能改——後端的 `EDITABLE_FIELDS` 全部允許。

    **這是 V2.3 那支修復的同一種形狀**（`48869a9`：「卡片的執行設定只能看不能改，
    而那讓派工從 console 走不通」），在同一個檔案上重演了一次。

20. 🔴 **`GATE-DV-NO-STALE-PROMISE` 漏掉了兩句話，而它們在最貴的位置。**
    畫面上寫著「這個交付方式從 V2.4 起生效，現在派工會被拒絕」，
    就印在一個**現在真的能用**的控制項旁邊。gate 的 pattern 比對
    `V2.4 會|將|adds|will`，而畫面用的是「V2.4 **起生效**」——
    **它抓到了用某一種語氣寫的承諾，抓不到用另一種語氣寫的。**

    ⚠️ 把 pattern 放寬成裸的 `V2.4 起` 之後，立刻抓到一句**不是**過期承諾的話：
    「自 V2.4 起它的結果必須是四個值之一」——那是在說一條規則從何時開始，永遠為真。
    所以動詞終究是訊號，只是清單要完整：**承諾說「起生效／會／將／之後才」，
    歷史說「自…起」**。兩種形狀現在都寫在 gate 的註解裡。
