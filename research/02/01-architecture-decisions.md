...
>
> 這一句要進 **ADR 0032**（交付模式與 PR 建立），否則 V2.4 開工時會有人拿上面那段
...
> | 2 | **D10** | 採納。內文 §「平台代跑驗證命令的否決理由」與本表原本互相矛盾，**定案措辭**：否決的是「讓呼叫端指名命令的 API」，允許的是「daemon 在 run 目錄內執行卡片宣告的驗證命令」 | D10 段尾的註記；ADR 0032 |
