...
**這是 D1 翻面新產生的問題**，原方案沒有（檔案在本機，離線照樣寫）。

情境：Central 掛了、網路斷了、node 的 WSS 斷線。此時 Agent 正在一個 Session 裡工作，它想更新計畫、提交驗證，但 `cliora` CLI 打不到平台。

**選項**：

| | A：CLI 直接失敗 | B：本機佇列，恢復後補送 | C：本機檔案 fallback，平台事後匯入 |
|---|---|---|---|
| Agent 體感 | 工具壞了，可能就放棄記錄 | 無感 | 無感 |
| 資料一致性 | 完好 | 補送順序要處理 | **兩個真實來源，D1 白翻了** |
| 實作成本 | 無 | 中（`.cliora/queue/` ＋ 補送） | 高 |

**建議：V2.1 用 A ＋ 明確錯誤訊息，V2.4 加 B。C 否決。**

> **Agent Run 路徑不適用這一條**：run 是平台派出去的，平台不在就不會有 run。D14 只影響**互動式 Session 裡**的 Agent。

A 的訊息要具體到可行動：「無法連線到 Cliora（Session 可繼續工作）。你的變更未被記錄，恢復連線後請重新執行。」——關鍵是第二句：**平台掛掉不影響 CLI Agent 本身的工作**，那是 V1 的既有性質，不能因為 V2 而改變。

B 的佇列寫在 `.cliora/queue/`，只 append、恢復後依序補送、補送成功即刪除。補送的資料 `source` 仍由伺服器判定，不因為它排過隊就升級。

C 否決的理由：它把 D1 的翻面白做了一次——一旦本機檔案可以被匯入為權威資料，就有兩個真實來源與衝突語意。

**這條要你裁決**：是否接受 V2.1 期間「平台掛掉 = 該次工作不留紀錄」。
