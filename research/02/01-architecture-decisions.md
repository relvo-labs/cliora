...
> **2026-08-08 裁決（例外 1／3）：採納 D7，但「代打第一行指令」整條刪除，不留成 opt-in。**
>
> 它要付的代價很具體：一個新的稽核語意（`actor = 建立者、on_behalf = platform`——
> 既有 `session_connections` 只記 writer 是誰）、一個送出前的內容確認 UI、
> 一個送出後的橫幅，以及「只送一次、不重試、不排隊」這組時序條件。
> 它買到的是**使用者少按一次 Enter**。
>
> 而下方原文自己已經寫了：「**Agent Run 路徑不需要它**——平台本來就是那次執行的啟動者」。
> 所以它只服務互動式 Session，而互動式 Session 的定義就是**有人在鍵盤前面**。
>
> **保留的部分不變**：檔案為預設、情境包內容從 DB 產生、env 注入否決（紅線 1／SEC-002）、
> `.cliora/` 的 30 天保留期。
>
> 下方「代打的完整語意」一段**保留為歷史**，不刪除；ADR 0027 的 rejected 表要有它一列。

**不變**：檔案為預設，~~「代打第一行指令」為 opt-in 且預設關~~（**已刪除，見上方裁決**），env 注入否決（紅線 1，SEC-002）。
...
> **2026-08-08 裁決（例外 2／3）：上面這一段與本文件的裁決表（§決策裁決表 D10 列）
> 互相矛盾，以下為定案措辭。**
>
> 兩句其實不矛盾，是同一件事的兩種說法沒對齊：
>
> | | 否決的 | 允許的 |
> |---|---|---|
> | 是什麼 | **一條讓呼叫端指名命令的 API** | **daemon 在 run 目錄內執行卡片宣告的驗證命令** |
> | 命令從哪來 | 請求 payload | 卡片，而卡片在平台 DB |
> | 誰決定執行 | 呼叫端 | daemon |
> | 適用路徑 | 全部 | **僅 Agent Run** |
>
> **定案的一句話**（與 D23 修訂後的不變式完全一致）：
>
> > 沒有任何請求 payload 能命名一個命令。驗證命令來自任務卡，由 daemon 在隔離目錄內執行，
> > exit code 為真。**這不構成一條「對任意 node 執行任意命令」的 API**，而後者仍然否決。
>
> 這一句要進 **ADR 0033**（交付模式與 PR 建立），否則 V2.4 開工時會有人拿上面那段
> 原文說「這條被否決了」。互動式 Session 路徑**完全不變**：平台不代跑任何東西。
...
**2026-08-09 修訂：V2.1 就隨 `agentd` 附帶，而且 `cliora` 就是 `agentd` 那支二進位**（argv[0] 分派 ＋ 安裝時的 symlink）。投影一支二進位在這個 repo 上做不到：單檔上限 4 MiB、`.cliora/` 對使用者寫入 verb 是禁區、update 的解壓器只取單一成員 `agentd`。**所以本節要的「穩定可執行路徑」在 V2.1 就已經成立**，V2.4 做 MCP 時不必再搬一次家（`plan/17/00-execution-plan.md` D2）。
...
- `task.approve` **獨立於 `task.update`**，因為核准與編輯是不同的權力。Agent token 永遠拿不到這一個，這是 D10 完整性錨點的一部分。
  ...
  > **2026-08-08 裁決（例外 3／3）：歸屬不變，但把理由寫對。**
  >
  > 上表裡這兩個動作的**持有者集合完全相同**（都是 Developer ＋ Admin）。
  > 所以拆開在角色層沒有分離任何東西——它唯一的實效是
  > **Agent 的 Session token scope 寫死不含它**。
  >
  > 定案措辭（要進 ADR 0027 與 `docs/permission-matrix.md` 的 render 對照表）：
  >
  > > `task.approve` 與 `task.update` 的持有者集合**刻意相同**。拆開不是為了角色分離，
  > > 是為了讓「Agent 憑證拿不到核准權」成為一個**可以寫死在 token scope 裡的東西**——
  > > 一個不存在的動作沒辦法從 scope 裡排除。
  >
  > 不寫這句，日後會有人提「這兩個動作可以合併，反正持有者一樣」，
  > 而那個合併會**靜默打開 Agent 自我核准的路**。
  >
  > 不改歸屬的理由：把 `task.approve` 上收給 Admin 會讓每一張卡的每一個 gate
  > 都要等 Admin，那會讓內化的 Review Gates 變成一個沒有人用得起的東西。
...
> **2026-08-08 修訂：V2.0 不使用 Traqora，用 `scripts/e2e/run-stack.sh` 既有的合成 workspace。**
>
> D30 原本把 V2.0 與 V2.1 併為「正式 repo 可以」。但 V2.0 的驗收內容
> （建 Project、綁兩個 node 的目錄、開 Session、看時間軸）**與 repo 裡有什麼完全無關**——
> 它一行程式碼都不讀，連 `.cliora/` 都不寫（那是 V2.1 才開始的）。
>
> 而 Traqora 真正的價值要更晚才兌現：
>
> | 價值 | 兌現於 |
> |---|---|
> | 真實待辦撐不撐得住六車道與 DoR | **V2.1** |
> | `base_branch: main` 逼出「不是寫死的 `master`」 | **V2.3** |
> | `p4-`／`p5-1-` 分支慣例不撞 `cliora/` 命名空間 | **V2.3** |
> | 第一個真的 PR | **V2.4** |
>
> 在 V2.0 用它，得到的只是「一個目錄名稱叫 traqora」。用 e2e 既有的合成 workspace
> 反而更好：可重現、CI 跑的是同一份、而且**兩台 node 的 workspace 可以刻意不同**
> （驗跨 node 綁定不依賴同一份程式碼）。**D30 的第一次真正使用是 V2.1。**
...
> **2026-08-08 裁決：本表 19 條「建議採納」一次全數採納，三條例外如下。**
>
> | 例外 | 決策 | 裁決 | 寫在哪 |
> |---|---|---|---|
> | 1 | **D7** | 採納，但**「代打第一行指令」整條刪除**，不留成 opt-in。檔案投影、情境包從 DB 產生、env 注入否決、`.cliora/` 30 天保留期全部不變 | D7 段首的註記 |
...
| 編號 | 主題 | 建議 | 裁決 |
|---|---|---|---|
| D3 | 看板詞彙 | 用 Monstrare `stage`，UI 中文標籤 | ✅ 採納 |
| D4 | Epic／User Story | 升級為平台實體 | ✅ 採納 |
| D7 ⚠️ | 情境交付 | 檔案為預設；「代打第一行」在 Agent Run 路徑上不需要（平台本來就是啟動者） | ✅ 採納（**例外 1**：代打第一行刪除） |
| D8 | 情境內容 | ≤4 KB，只給路徑 | ✅ 採納 |
| D9 | 執行計畫形狀 | DB 版本列，保留 append-only 語意 | ✅ 採納 |
| D10 ⚠️ | 證據可信度 | 三級 ＋ 伺服器端判定；「平台代跑驗證命令」在 Agent Run 路徑上**改為允許**，因為那本來就是 run 在做的事——但仍不開放「對任意 node 執行任意命令」的 API | ✅ 採納（**例外 2**：措辭定案，見 D10 段尾） |
| D11 ⚠️ | 工具介面 | CLI 優先，V2.1 首發。**MCP transport 已裁決為 stdio**（同一支二進位的子命令，隨 `agentd` 附帶，只在 Agent Run 路徑自動配置）；**做不做仍由 M2／M5 決定** | ✅ 採納 |
| D12 | 相容機制 | 單一旗標 ＋ 永久 nullable | ✅ 採納（**V2.0 必要**） |
| D31 🆕 | Mockup 預覽 | ✅ **已裁決：走既有 tunnel 整合（目前只有 Pinggy）**。Agent 在卡片上問策略、人決定、平台開。**未啟用整合時系統不做 mockup**：`ui` gate 自動停用、產出變體的卡在 dispatch 當下被拒、一般 UI 卡照常執行。截圖是 D29 的通用能力，不受影響 | —（已裁決） |
| D30 🆕 | 驗收素材 | ✅ **已裁決：用 Traqora**，不用 Cliora 自己。V2.2 期間限用 scratch clone（run 尚未隔離） | —（已裁決，階段表見下方註記） |
| D14 🆕 | 平台不可用時 | ✅ **已裁決：直接失敗，不做離線佇列**。關鍵在訊息要說出「Session 可繼續工作」；M9 是重新評估的觸發條件 | —（已裁決） |
| D13 | RBAC | 新增動作（見 `07` §5，已因 Runner 擴充） | ✅ 採納（**V2.0 必要**；**例外 3**：理由改寫） |
...
| D23 ⚠️ | SEC-002 修訂 | 只撤銷 env 值那一句，argv 完全保留 | ✅ 採納 |
| D24 | 看板溝通 | 訊息表 ＋ `waiting_for_input` ＋ 逾時退回 | ✅ 採納 |
| D25 ⚠️ | 自主執行收斂 | 限制出口而非限制沙箱內行為 | ✅ 採納 |
| D26 | Session 與 Run 的關係 | 並存，不做互轉 | ✅ 採納 |
| D27 ⚠️ | Run log | 存，但與互動 Session 的承諾嚴格分開 | ✅ 採納 |
