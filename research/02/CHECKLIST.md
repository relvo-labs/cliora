...
| ☐ | 編號 | 問題 | 擋住什麼 |
|---|---|---|---|
| ☐ | — | **機密加密主金鑰放哪**：環境變數、雲端 KMS，還是兩者都支援 | **V2.3 無法開工** |
| ☐ | — | **git 認證優先支援哪一種**：GitHub App、PAT，還是 SSH deploy key | **V2.3 無法開工** |
| ☐ | D14 | 互動式 Session 裡平台不可用時，Agent 寫入直接失敗（V2.1）還是進佇列（V2.4） | V2.1 的錯誤訊息設計 |
| ☐ | — | Cliora 自己要不要成為第一個被管理的 Project | V2.0 的驗收素材 |
| ☐ | — | RQ-07 的 mockup 預覽：只下載，還是 sandbox iframe ＋ CSP | V2.5 的 RQ-07（可整包延後） |
| ☐ | — | MCP 外殼要不要做（看 M2 的 CLI 使用率再決定） | V2.4 的一個工作包 |

### 1.2 建議採納但需點頭的決策

D3 看板詞彙 · D4 Epic/US 升為實體 · D7 情境交付 · D8 情境預算 · D9 計畫形狀 · D10 證據分級 · D11 工具介面 · D12 相容機制 · D13 RBAC · D15 流程可設定性 · D16 Agent 實體 · D17 認領模型 · D18 三層綁定 · D19 隔離目錄 · D20 Git 存取 · D22 機密管理 · D23 SEC-002 修訂 · D24 看板溝通 · D25 自主執行收斂 · D26 Session 與 Run 關係 · D27 Run log 界線
...
上線後觀察：M2（**Agent 實際使用 CLI 的比例——內化路線的核心假設**）、M3、M4、M5、M7、M9、M10、M13、M14、M15、M16。
...
前置：V2.2 出口通過、§1.1 的兩個金鑰／認證問題已答、M11／M12 已量。
...
| ☐ | RQ-07 | UI Mockup 關卡（**可延後**；卡在 HTML 產物的渲染決策） |
