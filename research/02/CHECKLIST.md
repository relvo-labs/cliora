...
| ☐ | — | RQ-07 的 mockup 預覽：**建議 D（截圖為主）**，或 A 只下載／B sandbox iframe／C 獨立 origin | V2.5 的 RQ-07（可整包延後） |
| ☐ | — | MCP 外殼要不要做 → **已改成可判定的規則**（`01` D11）：看 M2 使用率與 M5 格式錯誤率，兩者都好就整包砍掉 | V2.4 的一個工作包，**V2.1 上線後就要看數據** |
...
上線後觀察：M2（**Agent 實際使用 CLI 的比例——內化路線的核心假設，同時決定 MCP 做不做**）、M5（**與 M2 合看決定 MCP**）、M3、M4、M7、M9、M10、M13、M14、M15、M16。
