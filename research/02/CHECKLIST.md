...
| ☑ | — | ~~git 認證用哪一種~~ → **已裁決：fine-grained PAT ＋ SSH key 兩者都支援**（2026-08-08）。**2026-08-13 再裁決：兩者照做，但由 `CLIORA_GIT_SECRET_DELIVERY_ENABLED` 控制、預設關閉；現階段 git 認證以 node 端手動配置為準，平台不管**（`01` D20） | ~~V2.3~~ |
...
D3 看板詞彙 · D4 Epic/US 升為實體 · D7 情境交付 · D8 情境預算 · D9 計畫形狀 · D10 證據分級 · D11 工具介面 · D12 相容機制 · D13 RBAC · D15 流程可設定性 · D16 Agent 實體 · D17 認領模型 · D18 Project×Agent 配對（**2026-08-12 改寫：不綁定，改 tag 比對**）· D19 隔離目錄 · D23 SEC-002 修訂 · D24 看板溝通 · D25 自主執行收斂 · D26 Session 與 Run 關係 · D27 Run log 界線
...
前置：V2.1 出口通過、D16／D17／D17b／D18／D24／D26／D27／D29。（**D18 不影響本期**：綁定 2026-08-12 取消，tag 比對在 V2.3。）
...
> **執行計畫在 [`plan/20/`](../../plan/20/README.md)，ticket 編號以它為準**
> （本表的 SC-03…SC-06 在執行計畫裡拆得更細，並多了 `SC-00` 基線票與 `SC-07a`）。
> **2026-08-13 裁決**：git 憑證的下放由 `CLIORA_GIT_SECRET_DELIVERY_ENABLED` 控制、
> **預設關閉**；現階段 git 認證以 node 端手動配置為準，平台不管（`01` D20）。

| ☐ | Ticket | 內容 |
|---|---|---|
| ☐ | **SC-01** | **ADR 0032：機密管理與 SEC-002 修訂（先寫這一份，它決定其餘能否開工）**。**§0＝授權邊界是 enrollment**（D18：綁定表不做、爆炸半徑、代償四條） |
| ☐ | SC-02 | ADR 0031 增補：git **送回**半邊（五條硬約束 ＋ ambient 憑證的隔離開關**——只對真的收到平台憑證的 run 生效**，2026-08-13）。~~綁定授權~~ 2026-08-12 刪除 |
| ☐ | 🆕 SC-04b | **git 憑證下放的 flag 化**：`CLIORA_GIT_SECRET_DELIVERY_ENABLED` 預設 `false`；關閉時 git kind 的機密不能建立、`auth_kind` 只能 `ambient`、隔離不生效，而 clone 與 push 照常用 node 既有的憑證 |
| ☐ | 🆕 SC-02b | **ADR 0029 增補：tag 派工**（兩層配對、五條件資格判定、比對放 Central 的理由、四條 Alternatives rejected） |
| ☐ | 🆕 SC-02c | **tag 比對落地**：`agent_runners.run_untagged` migration ＋ contract v1.12.0（`runner.register` 加 `run_untagged`）＋ offer 查詢兩條件 ＋ dispatch 409 指名缺哪幾個 tag ＋ 「湊不齊 tag」的反向查詢 |
| ☐ | SC-03 | Secret store `0027`（**寫入後永不可讀回**） |
| ☐ | SC-04 | 下放與 **runner 端去識別** |
| ☐ | SC-05 | Daemon：隔離目錄、配額、repo mirror、git（`agentd` 0.10.0） |
| ☐ | SC-06 | 前端：Secrets、Repositories、卡片執行設定（**含 tag 輸入與線上 runner 的 tag 建議**）、Agents 頁 tag 改為「參與比對」＋ `run_untagged`、**enrollment 畫面的說明文字改寫**、Run 環境 |

**出口**：11 條主項 ＋ tag 派工的 6a–6f 六條（`10` §2.5）。**V2 風險最高的一階段**——同時觸發機密流、新儲存面、git 寫入三條，安全審查要分三節寫。
**2026-08-13 之後風險面小一格**：git 憑證流預設關閉，所以預設組態下平台管理的憑證只有 `kind: env` 那一種。安全審查第三節仍然要寫（git 寫入照做），但**憑證那一半要標明它驗的是一個預設關閉的路徑**。
**第四節是授權邊界**（2026-08-12）：綁定表這個檢查點消失了，改為驗代償四條真的存在。
...
| ☐ | 0029 | Agent Runner 模型與 run 生命週期；**V2.3 amend：tag 派工**（2026-08-12） | V2.2 |
| ☐ | 0030 | run 的兩種輸出：log 與卡片產物（保留期不同） | V2.2 |
| ☐ | 0031 | 隔離工作目錄 ＋ git **取得**（2026-08-10 提前）；**V2.3 增補** push（~~與綁定~~：2026-08-12 刪除） | **V2.2** |
