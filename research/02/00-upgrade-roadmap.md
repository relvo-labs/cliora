...
| 工作目錄 | 使用者選定的 allowed root | **每次執行一個隔離目錄**，daemon 擁有 |
...
| **V2.2** Agent Runner | Agent 註冊、多專案綁定、**自行認領**、租約與重排、run log 回傳、**產物附到任務卡**、看板上看得到執行中 | 認領無雙重領取；runner 死亡能重排；log 有界且去識別；產物有配額且安全提供 | V2.1 |
| **V2.3** 機密與隔離環境 | Secret store、安全下放、每次執行一個隔離目錄、依卡片 clone／checkout | 機密永不可讀回、永不入日誌；工作目錄有配額與清理；node 可拒絕 | V2.2 |
