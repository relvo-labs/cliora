...
**程式庫的檔案內容**由 Agent 在自己的執行環境中推送；
**平台不為了取得檔案內容而連向任何程式庫代管服務**。

> **2026-08-28 修訂（`plan/27` 的 `HD-01`，ADR 0043）。** 原文是
> 「程式庫內容由 Agent 在自己的執行環境中推送；**平台不主動連向任何程式庫代管服務**」。
>
> 後半句寫得比它要保護的東西寬，而且**寫下來的時候就已經不成立**：
> V2.4 起 `services/providers.py` 會主動連向 GitHub 開 pull request（ADR 0033 §3）。
> 這一條真正要保護的是「Central 沒有 git、不 clone、不讀檔案樹」——
> 那是 D77 的論證，而它完全不受 provider API 影響。
>
> `beta.2` 新增的是**唯讀的 PR／版本詮釋資料**（`FR-PROV-001`…`-004`），
> 它取得的不是檔案內容，而且**只在背景 worker 裡發生**
> （`GATE-HD-NO-PROVIDER-IN-REQUEST`）。檔案內容仍然只由 Agent 推上來。
>
> 修訂而不是靜靜違反：一條字面上被違反的需求，
> 下一個讀它的人會把整節當成已經沒有效力。
...
`beta.2`（V2-E1）新增的唯一一個對外副作用。全文論證在 ADR 0043。

一句話：

> 一個 PR 合掉了、一個版本發出去了——這兩件**發生在別人伺服器上**的事要能進到
> Project 記憶，帶著一個**低於任何經過 Cliora 核准的事實**的可信層級，
> 而整個過程平台**只發 GET**、**只在背景 worker 裡發**、**不新增任何未認證入口**。

<a id="fr-prov-001"></a>
## FR-PROV-001 唯讀取得，且只在背景

<a id="fr-prov-001-ac-01"></a>
平台向程式庫代管服務發出的請求**只有讀取**；建立、合併、核准、關閉與版本標記
都不在可達的動作表內。

<a id="fr-prov-001-ac-02"></a>
對外讀取**只發生在背景協調迴圈**，不發生在任何使用者請求的處理過程中。

<a id="fr-prov-001-ac-03"></a>
同步為**逐專案**設定，預設關閉；關閉時不發出任何對外請求。

<a id="fr-prov-001-ac-04"></a>
關閉同步**不刪除任何已取得的資料**——它停止更新，不清除記憶。

<a id="fr-prov-002"></a>
## FR-PROV-002 可信層級的上限

<a id="fr-prov-002-ac-01"></a>
未合併的 PR 取得**最低的討論層級**；已合併的 PR 與已發布的版本取得
**已審閱層級**，而那是 provider 資料能達到的最高層級。

<a id="fr-prov-002-ac-02"></a>
provider 資料**永遠無法**取得「經平台核准」或「經平台驗證」等級的層級——
別人的 CI 綠燈不是本平台的驗證。

<a id="fr-prov-002-ac-03"></a>
草稿版本不被收集：草稿不是一個事實。

<a id="fr-prov-002-ac-04"></a>
provider 內容進入情境包的**引用區**而非**指令區**；
內容中的祈使句不因此成為指令。

<a id="fr-prov-003"></a>
## FR-PROV-003 速率、失敗與停止

<a id="fr-prov-003-ac-01"></a>
每個程式庫每輪的讀取次數有上限；每小時的輪數由**已同步時間戳推導**，
不另建計數表。

<a id="fr-prov-003-ac-02"></a>
連續失敗達上限後該程式庫**停止同步**，而不是無限重試。

<a id="fr-prov-003-ac-03"></a>
停止的原因**在畫面上說得出來**——「沒有新的 PR」與「我們已經不再問了」
必須可區分。

<a id="fr-prov-003-ac-04"></a>
憑證沿用既有的專案機密機制，**永不下放到節點**；輪替後下一輪即使用新值。

<a id="fr-prov-004"></a>
## FR-PROV-004 新鮮度是一個承諾

<a id="fr-prov-004-ac-01"></a>
一個 PR 合併之後，在一個協調週期內出現在專案記憶中；
該週期長度是一個**寫進發行說明的數字**。

<a id="fr-prov-004-ac-02"></a>
延遲**可被量測**：協調落後時間是一個匯出的指標，
而不是一句沒有證據的宣稱。
...
前面幾節把一張卡片的事實建立起來：它的階段、它的執行、它的對話、它的驗證與它的記憶。
本節回答**看的那一半**：

> 同一份資料在 Board、Backlog、List、Task 詳情、My Work 與 Project 總覽六個畫面上
> 對「這張卡現在怎麼樣」給出的答案，由**同一段程式碼**產生；
> 而「哪些事情在等一個人類，而那個人類是我」有一個不必打開任何 Project 就能回答的入口。

範圍與取捨見 ADR 0040（生命週期投影與注意力的兩相位求值）與
ADR 0042（工作視圖的結構、範圍、權限與排序）。

> **本節不新增任何對外連線、任何新的憑證、任何新的權限動作，也不動 protocol。**
> 內容全部來自平台自己的資料庫，加上一項**只有這個行程知道**的事實：
> 哪些 node 現在連著。那一項不被儲存，理由見 FR-WORK-003。

本節能力屬於 `CLIORA_PROJECTS_ENABLED`。

<a id="fr-work-001"></a>
## FR-WORK-001 四個互相獨立的狀態面

<a id="fr-work-001-ac-01"></a>
一張卡片的狀態由**四個互相獨立**的面表達：工作進展、可執行程度、Agent 執行狀況、
以及是否等待人類的正式決定。四者各自回答一個問題，**任何一面的值不決定另一面的值**。

<a id="fr-work-001-ac-02"></a>
四個面**全部由既有欄位推導**，不新增任何一個儲存狀態的欄位；
同一張卡在任何畫面上四個面的值都相同。

<a id="fr-work-001-ac-03"></a>
既有的階段值一個都不移除，且每一個都能對應到一個工作進展值；
其中表示「被阻擋」的那個舊值，**永遠**投影成「被阻擋為真」，即使阻擋欄位另有說法。

<a id="fr-work-001-ac-04"></a>
一張已完成的卡片**永遠**不是被阻擋的。

<a id="fr-work-002"></a>
## FR-WORK-002 注意力有唯一的優先序

<a id="fr-work-002-ac-01"></a>
「這張卡需要一個人做什麼」以**一個有序清單**表達，順序在平台只定義一次；
畫面**不得**重新推導這個順序。

<a id="fr-work-002-ac-02"></a>
卡片上只顯示**最高**的那一項與同時成立的**項數**；完整清單在卡片詳情。

<a id="fr-work-002-ac-03"></a>
每一個注意力徽章除顏色之外**至少還有兩個線索**——文字、圖示或形狀——
且它的無障礙名稱是一個完整的說法，不是「注意」這類分類詞。

<a id="fr-work-002-ac-04"></a>
順序的第一位是「一個 Agent 停在那裡等一句話」，因為那是**唯一一個一句話就能解除**、
而且同時在消耗執行租約的狀態。

<a id="fr-work-003"></a>
## FR-WORK-003 兩項注意力不在資料庫裡，而平台說得出來

<a id="fr-work-003-ac-01"></a>
「沒有符合條件的執行機器」與「指定的執行機器離線」兩項，由**當下的連線狀態**決定，
平台**不儲存**這兩項的答案——一份存起來的副本在斷線時不會被更新。

<a id="fr-work-003-ac-02"></a>
因此這兩項**不可作為排序依據**，這件事在 API 的排序白名單上是可觀察的。

<a id="fr-work-003-ac-03"></a>
當連線狀態無法取得時，這兩項是**缺席**而不是「否」；
畫面必須能區分「沒有查」與「查過而且沒事」，且對應的篩選條件顯示為停用並說明原因，
**不得回傳零筆**。

<a id="fr-work-003-ac-04"></a>
對同一個排隊中的執行，**卡片畫面與單一執行的診斷畫面給出相同的答案**。

<a id="fr-work-004"></a>
## FR-WORK-004 篩選是白名單而不是查詢語言

<a id="fr-work-004-ac-01"></a>
可篩選的欄位與可用的運算子都是**列舉**；不在列舉內的一律以可辨識的錯誤碼拒絕，
而且錯誤**指名是哪一個欄位或哪一個運算子**。

<a id="fr-work-004-ac-02"></a>
條件的巢狀深度與條件總數各有上限；超過時以可辨識的錯誤碼拒絕並說出上限。

<a id="fr-work-004-ac-03"></a>
任何篩選都**不能**讓呼叫者看到他本來看不到的卡片；
決定「哪些專案看得到」的邏輯在平台只有**一處**。

<a id="fr-work-004-ac-04"></a>
篩選條件不以字串拼接進資料庫查詢。

<a id="fr-work-005"></a>
## FR-WORK-005 計數與清單說同一件事

<a id="fr-work-005-ac-01"></a>
每一欄的數字是**平台算的**，不是畫面上已載入的筆數。

<a id="fr-work-005-ac-02"></a>
計數與清單由**同一組條件**產生；對同一份資料，兩者給出相同的集合大小。

<a id="fr-work-005-ac-03"></a>
清單以游標分頁，且每一個分組各有自己的游標；載入更多不重複、不遺漏。

<a id="fr-work-005-ac-04"></a>
一頁卡片的回應大小有**量測過**的上限，且該上限記錄了它是在哪一份資料上量的。

<a id="fr-work-006"></a>
## FR-WORK-006 看板順序是穩定的

<a id="fr-work-006-ac-01"></a>
卡片在專案內有一個**明確的排序位置**；重新整理畫面不改變順序。

<a id="fr-work-006-ac-02"></a>
移動一張卡片時，用戶端送出的是**相鄰卡片的識別**而不是索引位置——
在有篩選的畫面上，索引位置指的不是同一件事。

<a id="fr-work-006-ac-03"></a>
一次移動只產生**一次**卡片寫入。

<a id="fr-work-006-ac-04"></a>
排序位置在密集到無法再插入時自動重新分配，且重新分配**不改變任何一張卡片的相對順序**。

<a id="fr-work-007"></a>
## FR-WORK-007 批次修改走與單張相同的那一條路

<a id="fr-work-007-ac-01"></a>
批次修改**逐張**經過與單張修改完全相同的規則：完成判準、相依檢查、樂觀鎖、
稽核與後續處理，一項都不繞過。

<a id="fr-work-007-ac-02"></a>
批次的授權是**逐張**判斷的；一個混合權限的批次整批被拒絕，不部分成功。

<a id="fr-work-007-ac-03"></a>
批次有筆數上限，且該上限是**量測交易成本之後**訂的；超過時以可辨識的錯誤碼拒絕。

<a id="fr-work-007-ac-04"></a>
同一個冪等鍵重送回傳原本的結果，不重複執行。

<a id="fr-work-008"></a>
## FR-WORK-008 存檔的視圖

<a id="fr-work-008-ac-01"></a>
視圖分為**個人**與**專案共用**兩種；共用視圖的建立與修改需要專案管理權限，
個人視圖只有擁有者能改。

<a id="fr-work-008-ac-02"></a>
一個視圖**不改變任何人的權限**：共用視圖若包含呼叫者無權查看的卡片，那些卡片不出現。

<a id="fr-work-008-ac-03"></a>
視圖可以指定顯示哪些欄位，而顯示欄位**只影響回應內容，不影響授權判斷**。

<a id="fr-work-008-ac-04"></a>
每個專案在建立時就有可用的預設視圖；改變哪一個是預設會留下稽核紀錄，
而個人的顯示密度偏好**不會**。

<a id="fr-work-009"></a>
## FR-WORK-009 臨時篩選不覆蓋存檔的視圖

<a id="fr-work-009-ac-01"></a>
快速篩選只改變當前網址，**不修改**任何存檔的視圖。

<a id="fr-work-009-ac-02"></a>
當前條件與存檔視圖不同時，畫面明確顯示「已修改」，並提供「另存為」與「還原」。

<a id="fr-work-009-ac-03"></a>
網址可分享：同一個網址在另一個有權限的人那裡呈現同一組條件。
條件過長而無法放進網址時，畫面明確說明「此篩選未包含在連結中」。

<a id="fr-work-009-ac-04"></a>
沒有指定任何條件時，套用該專案的預設視圖，而不是顯示未篩選的全部卡片。

<a id="fr-work-010"></a>
## FR-WORK-010 卡片詳情不打斷看板

<a id="fr-work-010-ac-01"></a>
打開一張卡片不離開看板：底層的視圖、篩選與捲動位置在關閉後**完全保留**。

<a id="fr-work-010-ac-02"></a>
當前打開的卡片記在網址裡：重新載入回到同一張卡片；瀏覽器的「上一頁」依序是
關閉卡片、還原視圖、離開專案。

<a id="fr-work-010-ac-03"></a>
無權查看的卡片顯示拒絕狀態且**不洩漏標題**；已刪除的卡片顯示不存在，
而底層的看板狀態保留。

<a id="fr-work-010-ac-04"></a>
鍵盤可完成打開與關閉；打開時焦點進入面板，關閉時焦點回到原來的位置。
面板**不使後方的看板失效**。

<a id="fr-work-011"></a>
## FR-WORK-011 對話在卡片詳情裡就能完成一輪

<a id="fr-work-011-ac-01"></a>
卡片在等待回覆時，詳情自動捲到並聚焦到問題與輸入框；
**不需要先展開執行紀錄才知道 Agent 在問什麼**。

<a id="fr-work-011-ac-02"></a>
「留言」與「回覆並繼續」在視覺上明確不同，且後者說明它會建立一個新的執行輪次。

<a id="fr-work-011-ac-03"></a>
回覆被保存但續跑被拒絕時，畫面明確說明**答覆已經寫下**，並顯示拒絕的原因代碼。

<a id="fr-work-011-ac-04"></a>
編輯衝突時**保留使用者已輸入的內容**，並提供重新載入與比對兩個動作。

<a id="fr-work-012"></a>
## FR-WORK-012 我的工作

<a id="fr-work-012-ac-01"></a>
有一個**不必打開任何專案**就能回答「哪些事情在等我」的入口，跨全部可見的專案。

<a id="fr-work-012-ac-02"></a>
它與單一專案的清單**共用同一組條件編譯與同一段注意力推導**，
只有專案範圍的條件不同。

<a id="fr-work-012-ac-03"></a>
每一個區段的條件只讀 Task、Run 與審查關卡的**真實狀態**；
沒有任何一段讀「使用者看過了沒有」。

<a id="fr-work-012-ac-04"></a>
畫面的新鮮度以固定間隔輪詢**計數**取得；分頁隱藏時停止，
且只有計數改變時才重新取得清單。
