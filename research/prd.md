...
以下兩條掛在 Terminal **顯示**底下而不是掛在視覺主題底下，是刻意的：它們是對終端的
要求，如果掛在主題功能上，主題功能哪天被移除，這兩條會跟著一起消失。

<a id="fr-term-001-ac-15"></a>
* 切換視覺主題後，終端 buffer、捲動位置、未送出的輸入與控制權四者不變，
  且不得重建終端（不得 `new Terminal()`、不得卸載持有 WebSocket 的面板）。
  終端與唯讀預覽必須與頁面同時換色，不得只有頁面換色。

<a id="fr-term-001-ac-16"></a>
* 終端字級可由使用者調整 12–20px 並持久化；調整後重新 fit 且不重建連線。
...
<a id="fr-term-005-ac-06"></a>
* 連線狀態必須在狀態列**分列**呈現，不得合併為單一指示：Session 狀態、
  瀏覽器連線狀態與控制權是三件事，而它們的組合有實際意義——
  「Session 執行中 ＋ 已斷線」不等於「Session 已結束」，第一種要按重新連線，
  第二種按了沒有用。合成一個綠色「正常」會讓這兩種情況長得一樣。
...
使用者可從瀏覽器把 Session 工作區裡的一個檔案取回本機。

與 `FR-FILE-002`（檔案預覽）是兩條不同的路徑，而且對同一個檔案可以有不同答案。
預覽回答「這個檔案能不能在編輯器裡顯示」，下載回答「這個檔案能不能原樣交出去」：
...
一句話：**平台不會交出它不肯顯示給你看的檔案，但會交出它只是無法呈現的檔案。**
前半句是安全規則，與 `FR-FILE-005` 共用同一份實作；後半句才是本需求要交付的東西。

本需求**只有「取得一個檔案」一個動詞**。不提供範圍請求、分塊下載、
下載資料夾或一次下載多個檔案；超過上限的檔案一律回到終端機或埠轉發處理。

<a id="fr-file-011-ac-01"></a>
持有 `file.browse` 的使用者可下載 Session 工作區內的單一檔案。

<a id="fr-file-011-ac-02"></a>
敏感檔案規則（`FR-FILE-005`）適用於下載，且與預覽共用同一份判定。

<a id="fr-file-011-ac-03"></a>
二進位檔案與非 UTF-8 文字檔可下載；`FR-FILE-004` 的判定不適用於下載。

<a id="fr-file-011-ac-04"></a>
單檔上限 4 MiB，逾越時明確拒絕並指出替代路徑，不得逾時或截斷。

<a id="fr-file-011-ac-05"></a>
請求只能指定工作區相對路徑；不得包含範圍、位移、編碼或內容型別等欄位。

<a id="fr-file-011-ac-06"></a>
回應一律以 `application/octet-stream` 加 `nosniff` 與 `attachment` 交付，
不得使用節點提供或嗅探所得的內容型別。

<a id="fr-file-011-ac-07"></a>
檔名以 `Content-Disposition` 交付，同時提供 ASCII 後備與 RFC 5987 的 UTF-8 形式。

<a id="fr-file-011-ac-08"></a>
每次成功下載留下稽核紀錄（使用者、Session、節點、相對路徑、位元組數），不記內容。

<a id="fr-file-011-ac-09"></a>
節點可停用此功能並回報；停用時前端不顯示下載入口。

<a id="fr-file-011-ac-10"></a>
中央不得保存下載內容：不落磁碟、不進資料庫、不進 log、不進 metrics label。
...
<a id="fr-conn-006-ac-12"></a>
* 檔案下載：20 秒（與圖片投放同長，理由是對稱的：Node 端要從磁碟讀出最多 4 MiB
  並做 base64 之後才回覆，而預覽只有 2 MiB 的文字。同樣是有上限的等待）。
...
在 `plan/28` 之前，這份文件的 115 條需求裡**沒有一條**在講可讀性、鍵盤操作或視覺主題。
後果不是那些要求被豁免，而是它們在追溯上根本看不見：2026-09-08 實測發現五處已經出貨的
對比失敗（全站焦點環 2.85:1、Terminate 確認鈕 4.09:1、所有輸入框邊界 1.37:1、
八個狀態標籤有六個低於 4.5:1），而沒有任何一條需求可以對它們變紅。本節補上那個維度。

對比一律是**實測值**，不是宣稱值：報告時必須標明具體的前景與背景，
不得以「看起來夠深」或單一色值宣布合格。

<a id="nfr-006-ac-01"></a>
* 使用者需要閱讀的文字，對其**實際**背景的對比不低於 4.5:1；
  必要控制邊界與焦點指示不低於 3:1。受驗配對的清單由**元件契約**產生
  （每個元件宣告它用到的配對），不是由 token 表產生——
  從 token 表出發會漏掉只在特定狀態下才出現的組合。

<a id="nfr-006-ac-02"></a>
* 每一個可聚焦控制項都有看得見的焦點指示，且不以 hover 代替鍵盤焦點。

<a id="nfr-006-ac-03"></a>
* 狀態不以顏色單獨表達；每一個狀態同時有可讀文字。

<a id="nfr-006-ac-04"></a>
* Dialog 開啟時焦點被限制在 Dialog 內，Esc 可關閉，關閉後焦點回到觸發它的元素。
  抽屜同此。

<a id="nfr-006-ac-05"></a>
* 圖示按鈕有可讀名稱（`aria-label`）與 tooltip；tooltip 不得取代 `aria-label`
  （tooltip 是給滑鼠的，`aria-label` 是給螢幕閱讀器的）。
  不得以文字符號充當導覽圖示。

<a id="nfr-006-ac-06"></a>
* 1440×900、1024×768 與 390×844 下核心操作可觸及，且沒有不必要的整頁橫向溢出。
  **可觸及不等於授權放寬**：終端輸入的權限不因裝置尺寸改變。

<a id="nfr-006-ac-07"></a>
* 視覺主題切換不改變任何授權、不中斷 Session、不重建終端。
  主題偏好只是顯示偏好；它不是能力。

<a id="nfr-006-ac-08"></a>
* `prefers-reduced-motion` 下停用非必要動畫；終端內容永不套用動畫
  （不淡入、不縮放、不打字動畫）。

<a id="nfr-007"></a>
## NFR-007 行動視窗的操作性

`NFR-006` 涵蓋的是「看不看得清楚」，這一節涵蓋的是「在手上那塊螢幕上做不做得到」。
兩者分開，是因為它們的驗證剖面不同：對比用算術驗，觸控目標與被鍵盤遮擋要量幾何，
而混在同一條需求底下，追溯報表就看不出哪一半有缺口。

`plan/29` 之前沒有任何一條需求可以對「控制項只有 36px 高」、
「面板在某個寬度區間完全叫不出來」或「軟體鍵盤蓋住輸入」變紅。
實際發生過兩件：1024–1100px 之間檔案欄在 DOM 裡、`display: none`、
且沒有任何控制可以開啟它；以及 node 姿態在壓縮標頭下被收進揭露之後，
而 ADR 0023 D10 要求它在使用者打字前就看得見。

<a id="nfr-007-ac-01"></a>
* 每一個可見的互動控制項，其命中區在任一驗收寬度下都不小於 44×44 CSS px。
  命中區可以大於視覺外框，但必須是**量出來**的，不是由樣式推論的。

<a id="nfr-007-ac-02"></a>
* 任一驗收寬度下，頁面層級都沒有水平溢位。內容自身的橫向瀏覽
  （終端輸出、寬表格）必須發生在它自己的捲動容器內，並且是明確的。

<a id="nfr-007-ac-03"></a>
* 任一功能不得只因為視窗寬度而變得無法取得。面板要嘛佔位，
  要嘛有一個可聚焦的控制項可以把它叫回來；沒有第三種狀態。

<a id="nfr-007-ac-04"></a>
* 版面斷點只有一組來源。同一個決定不得在樣式表與程式碼中各寫一次。

<a id="nfr-007-ac-05"></a>
* 視窗高度只有一個擁有者。軟體鍵盤升起時可用高度必須跟著縮，
  而安全區內距不得被重複相減。

<a id="nfr-007-ac-06"></a>
* 行動端的資訊不得因為版面壓縮而消失：可以改變排列、縮短字樣、
  改用截斷加 `title`，但不得移除欄位；安全姿態一律不得收進揭露之後。

<a id="nfr-007-ac-07"></a>
* 工作區相對路徑不得進入瀏覽器歷史、網址或任何持久化儲存。
...
18. Web 端上傳與下載檔案。
    ...
    *（2026-09-16：下載已由 `FR-FILE-011` 交付，本項的三片全數落地，不再是未來擴充。
    留在範圍外的是**分塊／範圍下載、資料夾下載與多檔下載**，見 ADR 0028 §9。
    這三項不是同一件事的剩餘部分，而是各自需要自己的機制。）*
...
7. 檔案總覽的**預覽**為唯讀；工作區的寫入僅經由平台明確定義的路徑，且每一條路徑
   ...
   *（2026-08-03 修訂：已建置的寫入路徑有兩條 —— 圖片投放 `FR-FILE-009` 與
   ...
   依產品決定改由 CLI／終端機承擔。）*
   *（2026-09-16 修訂：取出工作區位元組的路徑有一條 —— 下載 `FR-FILE-011`。
   它同樣經 `workspace.Root` 侷限、同樣每次留稽核、同樣可由節點拒絕並回報，
   但**沒有配額**：配額防的是磁碟被填滿，而讀取不消耗任何會用完的東西。
   讀取面的對應限制是敏感檔案政策與 RBAC，不是計數器（ADR 0028 §10）。）*
