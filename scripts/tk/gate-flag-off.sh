#!/usr/bin/env bash
# GATE-TK-FLAG-OFF: with the flag off, V2.1 must not exist.
#
# Not only the routes: a deployment with the project layer switched off must also issue
# no credential and send no projection, which is what the last two assertions cover.
set -euo pipefail
...
uv run --project backend pytest -q \
  backend/tests/db/test_tasks_api.py::test_every_task_route_is_404_while_the_flag_is_off \
