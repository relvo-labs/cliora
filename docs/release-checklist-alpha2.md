...
| ☑ | Annotated tags | Both created 2026-08-21: `v2.0.0-alpha.1` → `f91d9c4`, `v2.0.0-alpha.2` → `139f143`. Local only. |
| ☑ | Re-run the evidence **after** committing | Done, and it was not a formality: the commit made every artefact stale and `GATE-CE-EVIDENCE-FRESH` said so, so the whole pack was re-run at the commit being tagged (7 passed / 0 failed / 0 skipped) before the tag was created. It also caught a real gap — the `CE-01` preflight was not in the stack sequence, so its evidence went stale on its own. |
